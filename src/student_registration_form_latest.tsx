...
import { collection, addDoc, query, where, getDocs, orderBy, limit, doc, updateDoc, increment, arrayUnion } from "firebase/firestore";
...
const StudentRegistrationForm = () => {
  ...
  // Add function to fetch latest batch
const fetchLatestBatch = async () => {
 ...
 if (batchData.status === "active") {
  ...
  // Add useEffect to fetch batch when component mounts
  useEffect(() => {
    fetchLatestBatch();
  }, []);
  ...
  const saveStudentToFirebase = async () => {
    ...
    try {
      ...
      const studentData = {
        ...
        batchFirebaseId: latestBatch?.id || null,
      ...
      // ADD THIS: Update batch student count
      if (latestBatch?.id) {
          ...
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden max-w-2xl w-full">
                    ...
                    <div>
                      <div className="flex gap-2 mb-4">
                        {[1, 2, 3, 4, 5].map((level) => (
                          <div
                            key={level}
                            className={`h-2 flex-1 rounded-full transition-all ${passwordStrength.score >= level
                ...
                <div>
                  {latestBatch && (
                    <div className="mb-4 p-4 bg-purple-50 border border-purple-200 rounded-xl">
                      <p className="text-sm text-gray-600 mb-1">You will be enrolled in:</p>
                      <p className="text-lg font-semibold text-purple-700">{latestBatch.name}</p>
                    </div>
                  )}
                  <input
            ...
            <div className="px-12 pb-12">
              <button
                ...
                className={`w-full py-4 rounded-xl font-medium text-lg transition-all shadow-lg font-sans flex items-center justify-center ${canContinue
