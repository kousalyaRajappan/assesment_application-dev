...
if (!firebaseId) {
  ...
  const submitAssessmentWithAnswers = () => {
    try {
  ...
  const viewMyAnswers = (submissionId) => {
    ...
    setSelectedSubmission({ ...submission, assessment });
    setShowAssessmentModal(true);
  };
  ...
  const viewDetailedResults = (submissionId) => {
    ...
    setSelectedSubmission({ ...submission, assessment, showDetailedResults: true });
    setShowAssessmentModal(true);
  };
    ...
    <div className="card">
      ...
      <div style={{ maxWidth: '400px', margin: '0 auto' }}>
        ...
        <div className="form-group">
          ...
          <input ref={usernameRef} type="text" id="username" placeholder="Enter your username" />
        </div>
  ...
  const renderManageStudents = () => (
    <div>
      ...
      <div className="form-group">
        <label htmlFor="newStudentName">Add New Student:</label>
        <div style={{ display: 'flex', gap: '10px' }}>
          <input ref={newStudentNameRef} type="text" id="newStudentName" placeholder="Enter student name" />
      ...
      <div>
        {students.map(student => (
          <div key={student} className="question-card">
            <div className="question-header">
              <h4>👤 {student}</h4>
              <button className="btn btn-danger" onClick={() => removeStudent(student)}>
                Remove
              </button>
            </div>
          </div>
          ...
          myAssessments.map(assessment => {
            ...
            if (now >= start && now <= end) {
              ...
              if (existingSubmission && !existingSubmission.isDraft) {
                ...
                actionButton = (
                  <button className="btn" onClick={() => takeAssessment(assessment.id)}>
                    Take Assessment
                  </button>
                );
              }
            ...
            return (
              <div key={submission.id} className="question-card">
                ...
                <div style={{ marginTop: '15px' }}>
                  ...
                  {submission.isDraft && (
                    <button className="btn btn-warning" onClick={() => takeAssessment(assessment.id)}>
                      📝 Continue Editing
                    </button>
                  )}
                </div>
  ...
  const renderAssessmentModal = () => {
    ...
    // Original assessment taking modal
